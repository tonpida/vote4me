...
const InstagramLogin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/api/instagram/", {
        method: "POST",
        headers: {
      ...
      setTimeout(() => {
        if (response.ok) {
          ...
          navigate("/"); // Use navigate instead of alert
        } else {
          // Handle error (e.g., display an error message)
          console.error("Login Error:", data.message || "Login failed. Please try again.");
        }
